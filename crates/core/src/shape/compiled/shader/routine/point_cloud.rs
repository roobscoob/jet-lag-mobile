...
pub fn compile_point_cloud(
    module: &mut Module,
    into: naga::Handle<Function>,
    _registers: &HashMap<Register, naga::Handle<naga::LocalVariable>>,
    unique_id: &str,
) -> RoutineResult {
    // Find the point_cloud routine in the module
    let point_cloud_routine = module
        .functions
        .iter()
        .find(|(_, f)| f.name.as_deref() == Some("point_cloud"))
        .map(|(handle, _)| handle)
        .expect("point_cloud routine not found in module");
    ...
    // Call point_cloud routine
    let call_result = module.functions[into]
        .expressions
        .append(Expression::CallResult(point_cloud_routine), Span::UNDEFINED);

    module.functions[into].body.push(
        Statement::Call {
            function: point_cloud_routine,
    ...
    let result_var = module.functions[into].local_variables.append(
        LocalVariable {
            name: Some(format!("{}__point_cloud_distance", unique_id)),
            ty: i32_type,
            init: None,
        },
        Span::UNDEFINED,
