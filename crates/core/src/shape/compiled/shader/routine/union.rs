...
pub fn compile_union(
    ...
    move |module, into, register_map, unique_id| {
        // Get the local variables for each register
        ...
        // Build a balanced tree of min() calls using pre-created pointers
        fn build_min_tree(
            module: &mut Module,
            ...
            match ptrs.len() {
                0 => panic!("Cannot union zero values"),
                1 => {
                ...
                _ => {
                    // Split and recurse - builds min(min(a,b), min(c,d))
                    let mid = ptrs.len() / 2;
                    let left = build_min_tree(module, into, &ptrs[..mid]);
                    let right = build_min_tree(module, into, &ptrs[mid..]);
        ...
        let min_expr = build_min_tree(module, into, &var_ptrs);

        // Emit all the value expressions (Load and Min operations)
        ...
        let result_var = module.functions[into].local_variables.append(
            LocalVariable {
                name: Some(format!("{}__union_distance", unique_id)),
                ty: i32_type,
                init: None,
            },
            Span::UNDEFINED,
